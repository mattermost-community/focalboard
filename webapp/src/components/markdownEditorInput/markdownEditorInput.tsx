...
const MarkdownEditorInput = (props: Props): ReactElement => {
    ...
    const mentions: MentionData[] = useMemo(() => boardUsers.map((user) => ({name: user.username, avatar: `${imageURLForUser ? imageURLForUser(user.id) : ''}`, is_bot: user.is_bot, is_guest: user.is_guest})), [boardUsers])
