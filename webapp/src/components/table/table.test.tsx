...
describe('components/table/Table extended', () => {
    ...
    test('should delete snapshot', async () => {
        ...
        await waitFor(() => {
            expect(global.fetch).toHaveBeenCalledWith(`http://localhost/api/v1/workspaces/0/blocks/${card1.id}`, {headers: {Accept: 'application/json', Authorization: '', 'Content-Type': 'application/json', 'X-Requested-With': 'XMLHttpRequest'}, method: 'DELETE'})
        })
    })
})
