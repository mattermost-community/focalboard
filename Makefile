...
LDFLAGS += -X "github.com/mattermost/karmaboard/server/model.BuildNumber=$(BUILD_NUMBER)"
LDFLAGS += -X "github.com/mattermost/karmaboard/server/model.BuildDate=$(BUILD_DATE)"
LDFLAGS += -X "github.com/mattermost/karmaboard/server/model.BuildHash=$(BUILD_HASH)"
...
server: ## Build server for local environment.
	$(eval LDFLAGS += -X "github.com/mattermost/karmaboard/server/model.Edition=dev")
	cd server; go build -ldflags '$(LDFLAGS)' -tags '$(BUILD_TAGS)' -o ../bin/karmaboard-server ./main

server-mac: ## Build server for Mac.
	mkdir -p bin/mac
	$(eval LDFLAGS += -X "github.com/mattermost/karmaboard/server/model.Edition=mac")
ifeq ($(FB_PROD),)
	cd server; env GOOS=darwin GOARCH=$(MAC_GO_ARCH) go build -ldflags '$(LDFLAGS)' -tags '$(BUILD_TAGS)' -o ../bin/mac/karmaboard-server ./main
else
# Always build x86 for production, to work on both Apple Silicon and legacy Macs
	cd server; env GOOS=darwin GOARCH=amd64 CGO_ENABLED=1 go build -ldflags '$(LDFLAGS)' -tags '$(BUILD_TAGS)' -o ../bin/mac/karmaboard-server ./main
endif

server-linux: ## Build server for Linux.
	mkdir -p bin/linux
	$(eval LDFLAGS += -X "github.com/mattermost/karmaboard/server/model.Edition=linux")
	cd server; env GOOS=linux GOARCH=$(arch) go build -ldflags '$(LDFLAGS)' -tags '$(BUILD_TAGS)' -o ../bin/linux/karmaboard-server ./main

server-docker: ## Build server for Docker Architectures.
	mkdir -p bin/docker
	$(eval LDFLAGS += -X "github.com/mattermost/karmaboard/server/model.Edition=linux")
	cd server; env GOOS=$(os) GOARCH=$(arch) go build -ldflags '$(LDFLAGS)' -tags '$(BUILD_TAGS)' -o ../bin/docker/karmaboard-server ./main

server-win: ## Build server for Windows.
	$(eval LDFLAGS += -X "github.com/mattermost/karmaboard/server/model.Edition=win")
	cd server; env GOOS=windows GOARCH=amd64 go build -ldflags '$(LDFLAGS)' -tags '$(BUILD_TAGS)' -o ../bin/win/karmaboard-server.exe ./main

server-dll: ## Build server as Windows DLL.
	$(eval LDFLAGS += -X "github.com/mattermost/karmaboard/server/model.Edition=win")
	cd server; env GOOS=windows GOARCH=amd64 go build -ldflags '$(LDFLAGS)' -tags '$(BUILD_TAGS)' -buildmode=c-shared -o ../bin/win-dll/karmaboard-server.dll ./main

server-linux-package: server-linux webapp
	rm -rf package
	mkdir -p package/${PACKAGE_FOLDER}/bin
	cp bin/linux/karmaboard-server package/${PACKAGE_FOLDER}/bin
	...
	mkdir -p dist
	cd package && tar -czvf ../dist/karmaboard-server-linux-amd64.tar.gz ${PACKAGE_FOLDER}
	rm -rf package
...
server-linux-package-docker:
	...
	cp webapp/NOTICE.txt package/${PACKAGE_FOLDER}/webapp-NOTICE.txt
	mkdir -p dist
	cd package && tar -czvf ../dist/karmaboard-server-linux-$(arch).tar.gz ${PACKAGE_FOLDER}
	rm -rf package
...
server-test-mini-sqlite: export KARMABOARD_UNIT_TESTING=1
...
server-test-mysql: export KARMABOARD_UNIT_TESTING=1
server-test-mysql: export KARMABOARD_STORE_TEST_DB_TYPE=mysql
server-test-mysql: export KARMABOARD_STORE_TEST_DOCKER_PORT=44446
...
server-test-mariadb: export KARMABOARD_UNIT_TESTING=1
server-test-mariadb: export KARMABOARD_STORE_TEST_DB_TYPE=mariadb
server-test-mariadb: export KARMABOARD_STORE_TEST_DOCKER_PORT=44445
...
server-test-postgres: export KARMABOARD_UNIT_TESTING=1
server-test-postgres: export KARMABOARD_STORE_TEST_DB_TYPE=postgres
server-test-postgres: export KARMABOARD_STORE_TEST_DOCKER_PORT=44447
...
mac-app: server-mac webapp ## Build Mac application.
	...
	cp bin/mac/karmaboard-server mac/resources/bin/karmaboard-server
	cp app-config.json mac/resources/config.json
	cp -R webapp/pack mac/resources/pack
	mkdir -p mac/temp
	xcodebuild archive -workspace mac/Karmaboard.xcworkspace -scheme Karmaboard -archivePath mac/temp/karmaboard.xcarchive CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED="NO" CODE_SIGNING_ALLOWED="NO" \
		|| { echo "xcodebuild failed, did you install the full Xcode and not just the CLI tools?"; exit 1; }
	mkdir -p mac/dist
	cp -R mac/temp/karmaboard.xcarchive/Products/Applications/Karmaboard.app mac/dist/
	# xcodebuild -exportArchive -archivePath mac/temp/karmaboard.xcarchive -exportPath mac/dist -exportOptionsPlist mac/export.plist
	cp NOTICE.txt mac/dist
	cp webapp/NOTICE.txt mac/dist/webapp-NOTICE.txt
	cd mac/dist; zip -r karmaboard-mac.zip Karmaboard.app MIT-COMPILED-LICENSE.md NOTICE.txt webapp-NOTICE.txt
...
linux-app: webapp ## Build Linux application.
	...
	rm -rf linux/dist
	mkdir -p linux/dist
	mkdir -p linux/temp/karmaboard-app
	cp app-config.json linux/temp/karmaboard-app/config.json
	cp NOTICE.txt linux/temp/karmaboard-app/
	cp webapp/NOTICE.txt linux/temp/karmaboard-app/webapp-NOTICE.txt
	cp -R webapp/pack linux/temp/karmaboard-app/pack
	cd linux; make build
	cp -R linux/bin/karmaboard-app linux/temp/karmaboard-app/
	cd linux/temp; tar -zcf ../dist/karmaboard-linux.tar.gz karmaboard-app
	rm -rf linux/temp
