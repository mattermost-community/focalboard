...
func Test34DropDeleteAtColumnMySQLPostgres(t *testing.T) {
	t.Run("column exists", func(t *testing.T) {
		...
		if th.IsMySQL() {
			var count int
			query := "SELECT COUNT(column_name) FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name = 'karmaboard_category_boards' AND column_name = 'delete_at'"
			th.f.DB().Get(&count, query)
			require.Equal(t, 0, count)
		} else if th.IsPostgres() {
			var count int
			query := "select count(*) from information_schema.columns where table_name = 'karmaboard_category_boards' and column_name = 'delete_at'"
			th.f.DB().Get(&count, query)
			require.Equal(t, 0, count)
		}
	})
