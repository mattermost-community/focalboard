...
func (n *notifier) notifySubscribers(hint *model.NotificationHint) error {
	...
	opts := DiffConvOpts{
		...
		MakeCardLink: func(block *model.Block, board *model.Board, card *model.Block) string {
			return fmt.Sprintf("[%s](%s)", block.Title, utils.MakeCardLink(n.serverRoot, board.ID, card.ID))
		},
		Logger: n.logger,
	}
	...
	if len(attachments) > 0 {
		for _, sub := range subs {
			...
			if err = n.delivery.SubscriptionDeliverSlackAttachments(hint.TeamID, sub.SubscriberID, sub.SubscriberType, attachments); err != nil {
