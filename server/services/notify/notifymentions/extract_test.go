...
func Test_safeConcat(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "out of range", want: s0 + s1 + s2 + s3 + s4 + s5 + s6 + s7, args: args{start: -22, end: 99, lines: all}},
		{name: "2,3", want: s2 + s3, args: args{start: 2, end: 4, lines: all}},
...
func Test_safeSubstr(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "good", want: "is line", args: args{start: 26, end: 33, s: s0 + s1 + s2}},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := safeSubstr(tt.args.s, tt.args.start, tt.args.end); got != tt.want {
				t.Errorf("safeSubstr() = [%v], want [%v]", got, tt.want)
			}
		})
	}
}
