...
| localModeSocketLocation | Location of local Unix port    | `/var/tmp/karmaboard_local.socket`
| enablePublicSharedBoards | Enable publishing boards for public access | `false`

## Resetting passwords

By default, personal server exposes admin APIs on a local Unix socket at `/var/tmp/karmaboard_local.socket`. This is configurable using the `enableLocalMode` and `localModeSocketLocation` settings in `config.json`.
...
curl --unix-socket /var/tmp/karmaboard_local.socket http://localhost/api/v2/admin/users/$1/password -X POST -H 'Content-Type: application/json' -d '{ "password": "'$2'" }'
```
